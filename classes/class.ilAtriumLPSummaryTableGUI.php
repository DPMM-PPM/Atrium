 ...
 *
 *
 * @ilCtrl_Calls ilAtriumLPSummaryTableGUI: ilFormPropertyDispatchGUI
 */
class ilAtriumLPSummaryTableGUI extends ilLPTableBaseGUI
{
	/**
	 * Constructor
	 */
	...
	protected function fillHeaderExcel(ilExcel $a_excel, &$a_row): void // VINCENT SAYAH
	{
		...
		foreach ($this->getSelectedColumns() as $c)
		{
			...
			if(!$this->isArrayColumn($c))
			{
				...
				$a_excel->setCell($a_row, $cnt, $label); // VINCENT SAYAH
				$cnt++;
			}
			else
			{
				if($c != "status")
				{
					...
					$a_excel->setCell($a_row, $cnt, $label." #1"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." #1"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." #1 %"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." #2"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." #2"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." #2 %"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." #3"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." #3"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." #3 %"); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." ".$this->lng->txt("trac_others")); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." ".$this->lng->txt("trac_others")); // VINCENT SAYAH
					$a_excel->setCell($a_row, ++$cnt, $label." ".$this->lng->txt("trac_others")." %"); // VINCENT SAYAH
				}
				else
...
//					$valid_status = array(LP_STATUS_NOT_ATTEMPTED_NUM, LP_STATUS_IN_PROGRESS_NUM, LP_STATUS_COMPLETED_NUM, LP_STATUS_FAILED_NUM);
					...
					foreach($valid_status as $status)
					{
						...
						$worksheet->write($a_row, ++$cnt, $text." %");*/
						$a_excel->setCell($a_row, ++$cnt, $text); // VINCENT SAYAH
						$a_excel->setCell($a_row, ++$cnt, $text." %"); // VINCENT SAYAH
					}
				}
				$cnt++;
			...
			else
			{
				foreach($a_set[$c] as $idx => $value)
				{
					if($c == "status")
					{
						...
						$a_excel->setCell($a_row, $cnt, (int)$value["absolute"]); // VINCENT SAYAH
						$a_excel->setCell($a_row, ++$cnt, $value["percentage"]); // VINCENT SAYAH
					}
					else
					{
						...
						$worksheet->write($a_row, ++$cnt, (int)$value["absolute"]);
						$worksheet->write($a_row, ++$cnt, $value["percentage"]); */
						$a_excel->setCell($a_row, $cnt, $value["caption"]); // VINCENT SAYAH
						$a_excel->setCell($a_row, ++$cnt, (int)$value["absolute"]); // VINCENT SAYAH
						$a_excel->setCell($a_row, ++$cnt, $value["percentage"]); // VINCENT SAYAH
					}
					$cnt++;
				...
				if(sizeof($a_set[$c]) < 4 && $c != "status")
				{
					for($loop = 4; $loop > sizeof($a_set[$c]); $loop--)
					{
						...
						$worksheet->write($a_row, ++$cnt, "");
						$worksheet->write($a_row, ++$cnt, "");*/
						$a_excel->setCell($a_row, $cnt, ""); // VINCENT SAYAH
						$a_excel->setCell($a_row, ++$cnt, ""); // VINCENT SAYAH
						$a_excel->setCell($a_row, ++$cnt, ""); // VINCENT SAYAH
						$cnt++;
					}
